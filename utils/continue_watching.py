...
def continue_watching():
    ...
    for i, anime in enumerate(available_list):
        print(f'[{GREEN}{i + 1}{re}] {CYAN}{anime["title"]}{re} - {YELLOW}Episode {int(anime["progress"]) + 1}{re}')
...
def more_options():
    while True:
        print(f"\n{GREEN}m{re} - map folders to AniList IDs")
        print(f"{GREEN}o{re} - add offset")
        print(f"{GREEN}w{re} - watch")
        print(f"{GREEN}q{re} - quit{re}")
        user_input = input("\nInput: ")
        if user_input == "m":
            utils.mapper.map()
        elif user_input == "o":
